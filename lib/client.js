window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region \0dsh-css-module:./src/client/ClaudeActivityView.module.css.mjs
		...
		const tagId = "dsh-llm-claude-agent-sdk/ClaudeActivityView.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-llm-claude-agent-sdk";
		...
		const claudeActivityDefinition = {
			...
			start: (_context, match) => {
				if (match.event.type !== "claude-agent-sdk/activity") throw new Error("Claude activity start requires claude-agent-sdk/activity");
				return {
