...
const CLAUDE_ACTIVITY_EVENT = "claude-agent-sdk/activity";
...
function installClaudeSessionEventType() {
	if (KNOWN_SESSION_EVENT_TYPES.has("claude-agent-sdk/activity")) return;
...
function resolveLinkPath(value) {
	const configured = value ?? process.env.DSH_CLAUDE_SDK_LINK_PATH;
	if (configured) return resolve(configured);
	return join(resolve(process.env.DSH_HOME?.trim() || join(homedir(), ".dsh")), "plugin-data", "claude-agent-sdk-links.json");
}
//#endregion
//#region host-plugin.js
const name = "dsh-llm-claude-agent-sdk";
const inject = [
	"agents",
	"approval",
	"llm",
